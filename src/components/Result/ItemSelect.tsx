...
export const ItemSelect: React.FC<ItemSelectProps> = (props) => {
    ...
    const ownedAssets: Array<Metadata> = useSelector(
    ...
    let currentOwnedAsset: Metadata = useSelector(
        (state: RootState) => state.currentOwnedAsset
    );
                    ...
                    <SelectionOptionsList>
                        {walletAssets.map((asset: Asset, i: number) => {
                            return (
                                <SelectionOptionsListItem
                                    key={i}
                                    hidden={currentAsset.name === asset.name}
                        ...
                        {ownedAssets.map((asset: Metadata, i: number) => {
                            return (
                                <SelectionOptionsListItem
                                    key={i}
                                    hidden={
                                        currentOwnedAsset.name === asset.name
                                    }
                                    onClick={() => handleSelectionClick2(i)}
                                >
                                    {asset.name}
                                </SelectionOptionsListItem>
                            );
